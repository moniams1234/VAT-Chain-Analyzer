...
def analyze_transaction(data: TransactionInput) -> RuleEngineResult:
    ...
    raw_organizer_idx = data.transport_organizer_index

    # Incoterms traktujemy jako przesłankę pomocniczą, nie jako automatyczne zastąpienie danych użytkownika.
    organizer_idx, incoterms_notes, incoterms_confidence = _adjust_for_incoterms(
        data=data,
        organizer_idx=raw_organizer_idx,
    )

    if incoterms_notes:
        requires_verification.extend(incoterms_notes)

    organizer = data.parties[organizer_idx]
    movable_idx = None
    movable_confidence = incoterms_confidence
    ...
    for i in range(num_deliveries):
        ...
        if is_movable:
            ...
            # Jeżeli Incoterms obniżył pewność przypisania transportu — przenieś to na dostawę ruchomą.
            d_confidence = _lower_confidence(d_confidence, movable_confidence)

            if incoterms_notes:
                d_notes += (
                    " Incoterms potraktowano jako przesłankę pomocniczą. "
                    "Ostateczne przypisanie transportu wymaga weryfikacji umowy, zlecenia transportowego, "
                    "faktury, dokumentów przewozowych i ustaleń między stronami."
                )
    ...
    requires_verification = list(dict.fromkeys(requires_verification))
...
def _adjust_for_incoterms(data: TransactionInput, organizer_idx: int):
    """
    Analiza Incoterms jako przesłanki pomocniczej przy ocenie organizatora transportu.

    Ważne:
    - Funkcja NIE powinna automatycznie i bezwarunkowo nadpisywać faktów wskazanych przez użytkownika.
    - Incoterms określają głównie podział kosztów, ryzyk i obowiązków handlowych.
    - Dla VAT kluczowe jest faktyczne przypisanie wysyłki/transportu do jednej dostawy
      w rozumieniu art. 22 ust. 2–2e ustawy o VAT.
    """
    notes = []
    confidence = ConfidenceLevel.HIGH
    incoterm = (data.incoterms or "").upper().strip()
    last_idx = len(data.parties) - 1

    # EXW — odbiór u sprzedawcy. Często transport organizuje nabywca,
    # ale w łańcuchu może to być podmiot pośredniczący albo końcowy nabywca.
    if incoterm == "EXW":
        confidence = ConfidenceLevel.MEDIUM
        notes.append(
            "Incoterms EXW może wskazywać, że transport organizuje nabywca odbierający towar od sprzedawcy. "
            "Nie należy jednak automatycznie przypisywać transportu wyłącznie na podstawie EXW. "
            "Zweryfikuj, kto faktycznie zlecił transport, poniósł jego koszt i na czyją rzecz działał przewoźnik."
        )
        return organizer_idx, notes, confidence

    # FCA — sprzedawca wydaje towar przewoźnikowi wskazanemu przez kupującego.
    if incoterm == "FCA":
        confidence = ConfidenceLevel.MEDIUM
        notes.append(
            "Incoterms FCA wymaga weryfikacji, który podmiot wskazał przewoźnika i na czyją rzecz działał przewoźnik. "
            "Może to mieć wpływ na przypisanie dostawy ruchomej."
        )
        return organizer_idx, notes, confidence

    # CPT/CIP — sprzedawca opłaca przewóz do miejsca przeznaczenia, ale ryzyko przechodzi wcześniej.
    if incoterm in ["CPT", "CIP"]:
        confidence = ConfidenceLevel.MEDIUM
        notes.append(
            f"Incoterms {incoterm} oznacza, że sprzedawca zasadniczo zawiera umowę przewozu do określonego miejsca, "
            "ale przejście ryzyka może nastąpić wcześniej. Dla VAT wymaga to weryfikacji dokumentów transportowych."
        )
        return organizer_idx, notes, confidence

    # DAP/DPU/DDP — sprzedawca odpowiada za dostarczenie do miejsca przeznaczenia.
    if incoterm in ["DAP", "DPU", "DDP"]:
        confidence = ConfidenceLevel.MEDIUM
        if organizer_idx == last_idx:
            notes.append(
                f"Incoterms {incoterm} zwykle wskazuje, że sprzedawca odpowiada za dostarczenie towaru do miejsca przeznaczenia, "
                "podczas gdy użytkownik wskazał końcowego nabywcę jako organizatora transportu. "
                "To potencjalna sprzeczność wymagająca weryfikacji."
            )
        else:
            notes.append(
                f"Incoterms {incoterm} zwykle wskazuje na silniejszą rolę sprzedawcy w organizacji dostawy do miejsca przeznaczenia. "
                "Zweryfikuj, czy wskazany organizator transportu jest zgodny z umową, fakturą i dokumentami przewozowymi."
            )
        return organizer_idx, notes, confidence

    # FAS/FOB/CFR/CIF — reguły morskie, sensowne głównie przy transporcie morskim.
    if incoterm in ["FAS", "FOB", "CFR", "CIF"]:
        confidence = ConfidenceLevel.MEDIUM
        if data.transport_type.value != "morski":
            notes.append(
                f"Incoterms {incoterm} jest regułą właściwą dla transportu morskiego / wodnego. "
                f"Wybrano transport: {data.transport_type.value}. Zweryfikuj poprawność Incoterms."
            )
        else:
            notes.append(
                f"Incoterms {incoterm} wymaga weryfikacji dokumentów morskich, w szczególności konosamentu, "
                "oraz ustalenia, na czyją rzecz działał przewoźnik / spedytor."
            )
        return organizer_idx, notes, confidence
...
def _lower_confidence(primary: ConfidenceLevel, modifier: ConfidenceLevel) -> ConfidenceLevel:
    """Zwraca niższy poziom pewności z dwóch wartości."""
    order = {
        ConfidenceLevel.HIGH: 3,
        ConfidenceLevel.MEDIUM: 2,
        ConfidenceLevel.LOW: 1,
        ConfidenceLevel.REQUIRES_VERIFICATION: 0,
    }
    return primary if order.get(primary, 0) <= order.get(modifier, 0) else modifier
